...
def encrypt_to_public_key(
    ...
    print(f"TRACE_CRYPTO:ENCLAVE_ENCRYPT recipient_pub={recipient_public_key.hex()} context={context}", flush=True)
    print(f"TRACE_CRYPTO:ENCLAVE_ENCRYPT eph_pub={ephemeral.public_key.hex()}", flush=True)
    print(f"TRACE_CRYPTO:ENCLAVE_ENCRYPT shared_secret={_shared_secret.hex()}", flush=True)
    print(f"TRACE_CRYPTO:ENCLAVE_ENCRYPT derived_key={derived_key.hex()}", flush=True)
    print(f"TRACE_CRYPTO:ENCLAVE_ENCRYPT hkdf_salt={salt.hex()}", flush=True)
