...
class CreateOrgKeysRequest(BaseModel):
    ...
    """
    org_public_key: str = Field(
        ...,
        min_length=64,
        max_length=64,
        description="Org X25519 public key (32 bytes hex)"
    )
    ...
    admin_encrypted_private_key: str = Field(
        ...,
        min_length=1,
        description="Org private key encrypted with org passcode (hex)"
    )
    admin_iv: str = Field(
        ...,
        min_length=32,
...
class DistributeOrgKeyRequest(BaseModel):
    ...
    membership_id: str = Field(
        ...,
        description="Membership ID of the member to receive the key"
    )
    encrypted_org_key: EncryptedPayload = Field(
        ...,
...
class BatchDistributeOrgKeyRequest(BaseModel):
    ...
    distributions: List[DistributeOrgKeyRequest] = Field(
        ...,
        min_length=1,
        description="List of key distributions"
    )
...
class RevokeOrgKeyRequest(BaseModel):
    ...
    membership_id: str = Field(
        ...,
        description="Membership ID to revoke key from"
    )
    reason: Optional[str] = Field(
        None,
        max_length=500,
        description="Optional reason for revocation (logged for audit)"
    )
...
class ChangeMemberRoleRequest(BaseModel):
    ...
    membership_id: str = Field(
        ...,
        description="Membership ID to change role"
    )
    new_role: str = Field(
        ...,
