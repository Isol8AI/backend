"""
KMS envelope encryption for agent state storage.
...
from core.crypto.primitives import encrypt_aes_gcm, decrypt_aes_gcm
...
def encrypt_with_kms(plaintext_data: bytes, kms_key_id: str) -> Dict[str, bytes]:
    ...
    logger.debug(
        f"Encrypted {len(plaintext_data)} bytes with KMS envelope "
...
def decrypt_with_kms(envelope: Dict[str, bytes], kms_key_id: str) -> bytes:
    ...
    logger.debug(f"Decrypted {len(plaintext)} bytes from KMS envelope")
